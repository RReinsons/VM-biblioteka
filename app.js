(() => {
  ...
  function render(){
    $("#status").textContent=t("results")(state.filtered.length); const size=cfg.pageSize||12, pages=Math.max(1,Math.ceil(state.filtered.length/size)); state.page=Math.min(state.page,pages); const rows=state.filtered.slice((state.page-1)*size,state.page*size);
  ...
  $("#searchForm").onsubmit=e=>{e.preventDefault();state.page=1;apply()}; $("#query").addEventListener("input",()=>{state.page=1;apply()}); $("#sort").onchange=apply; $("#gridView").onclick=()=>setView("grid"); $("#listView").onclick=()=>setView("list"); $("#clearPins").onclick=()=>{state.pins.clear();localStorage.removeItem("vm-pins");apply()};
